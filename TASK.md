...
- 수출 플로우 기본 HS는 화장품 `330499`. 바이어 카드·상세 리포트·딜 후보는 API의 `source_file`/`source_row_no`만 표시하고, 파일명을 만들지 않는다.
- 없는 원천·미검증 출처·추정 연락처는 `REVIEW_REQUIRED`. 조건이 맞아도 딜은 `DRY_RUN`이며 메일을 보내지 않는다.
- 관세율·환율은 사용자 가정으로 표시한다.
- REQUEST_SOLVED=NO. 운영 화면에서 1품목 전체 클릭 재현과 원본 행 대조는 아직이다. SMTP 실발송과 P2 원본 CSV는 이 작업 범위 밖이다.
...
> 실제 바이어 데이터를 확인할 수 있게 연결해 테스크에 추가.

### 최종 결과

사용자가 바이어 검색 결과에서 한 회사를 열었을 때, 그 결과가 실제 `buyer_candidate`의 어느 원천 레코드에서 왔는지 화면에서 확인할 수 있다.

현재 전처리 코드에 이미 존재하는 원천 추적 필드인 `source_dataset`, `source_file`, `source_row_no`를 그대로 사용한다. 없는 출처값을 새로 만들어내지 않는다.

### MUST

- 바이어 검색 결과가 실제 `buyer_candidate` 레코드와 연결되어 있는지 먼저 추적한다
- 실제 레코드의 `source_dataset`, `source_file`, `source_row_no`를 검색/상세 API를 거쳐 기존 BuyerSearch 상세 화면까지 전달한다
- 사용자가 바이어 상세에서 데이터 출처와 원본 행 번호를 직접 확인할 수 있게 한다
- 가능하면 `source_file` + `source_row_no`로 `output/raw` 원본 CSV의 해당 행까지 대조 가능하게 한다
- 실제 데이터 연결이 없는 mock/demo/sample 결과를 실제 바이어로 표시하지 않는다
- 원천 추적정보가 없는 레코드는 `원천 확인 불가`처럼 명시하고 VERIFIED로 취급하지 않는다
- 입력값 누락, 잘못된 행 번호, 원본 파일 없음, 일부 출처 필드 없음, 로딩 중, 원천 조회 실패 상태를 처리한다

### KEEP

- 기존 바이어 검색/랭킹/FitScore 동작
- 기존 BuyerSearch 상세 흐름
- 기존 인증/사용자 격리
- 기업검증/연락처 검증 기능
- 현재 저장된 실제 데이터와 기존 스키마

### REMOVE

- 원천 근거 없이 화면에서 실제 데이터처럼 보이는 표시가 있다면 제거하거나 `원천 확인 불가`로 바꾼다

### FORBIDDEN

- 가짜 출처명, 가짜 URL, 가짜 원본 행 번호 생성
- 실제 데이터가 없는데 임의 buyer 생성
- 이번 TASK를 위해 새 유료 데이터 API 계약 또는 외부 스크래핑 추가
- 라이선스가 확인되지 않은 원본을 새로 커밋
- 관련 없는 검색 점수/랭킹/DB 구조 변경
- 새 전역 페이지/라우터 생성

### VERIFY

- 실제 사용자 흐름으로 로그인 → HS/제품 검색 → 바이어 1건 선택 → 상세에서 `source_dataset`, `source_file`, `source_row_no` 확인
- 화면/API에 표시된 세 값이 실제 `buyer_candidate` 레코드 및 가능한 경우 `output/raw` 원본 행과 일치하는지 1건 이상 대조
- 원천 추적정보가 없는 테스트 케이스에서는 `원천 확인 불가` 상태가 표시되고 실제 데이터로 오인되지 않는지 확인
- 관련 backend 테스트와 frontend build를 통과시키고 기존 바이어 검색 흐름 회귀가 없는지 확인
