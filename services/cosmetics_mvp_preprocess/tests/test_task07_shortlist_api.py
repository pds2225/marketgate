...
def test_tc_api_1_include_rejected_still_excludes_blocked_medical_device() -> None:
    output_dir = _build_output_dir()
    try:
        ...
        assert "Medical Device Co" not in item_names
        assert decisions_by_name["Serum Lab"] != "rejected"
    finally:
        shutil.rmtree(output_dir, ignore_errors=True)


def test_tc_api_2_banned_countries_excludes_matching_country_items() -> None:
    output_dir = _build_output_dir()
    try:
        _write_task07_policy_fixture(output_dir)
...
def test_tc_api_3_default_response_excludes_blocked_buyer() -> None:
    output_dir = _build_output_dir()
    try:
        _write_task07_policy_fixture(output_dir)
...
def test_tc_api_4_blocked_keyword_buyers_excluded_even_with_include_rejected() -> None:
    output_dir = _build_output_dir()
    try:
        ...
        assert "Medical Cosmetics Buyer" not in item_names
        assert "Beauty Equipment Buyer" not in item_names
        assert "Pharma Supplement Buyer" not in item_names
        assert "Serum Lab" in item_names
    finally:
        shutil.rmtree(output_dir, ignore_errors=True)
